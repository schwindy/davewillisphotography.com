...
class PaymentMethodTest extends Setup
{
    ...
    public function testCreate_fromFakeVenmoAccountNonce()
    {
        ...
        $this->assertEquals([], $venmoAccount->subscriptions);
    ...
    public function testCreate_handlesValidationErrorsForPayPalAccounts()
    {
        ...
        $this->assertEquals(Braintree\Error\Codes::PAYPAL_ACCOUNT_CANNOT_VAULT_ONE_TIME_USE_PAYPAL_ACCOUNT,
            $errors[0]->code);
        $this->assertEquals(Braintree\Error\Codes::PAYPAL_ACCOUNT_CONSENT_CODE_OR_ACCESS_TOKEN_IS_REQUIRED,
            $errors[1]->code);
    }
    ...
    public function testUpdate_updatesAPaypalAccountsToken()
    {
        ...
        $this->setExpectedException('Braintree\Exception\NotFound',
            'payment method with token ' . $originalToken . ' not found');
