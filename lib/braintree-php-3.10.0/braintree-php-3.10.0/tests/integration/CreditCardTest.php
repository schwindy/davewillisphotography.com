...
class CreditCardTest extends Setup
{
    ...
    public function testCreate_withCardVerification()
    {
        ...
        $this->assertFalse($result->success);
        $this->assertEquals(Braintree\Result\CreditCardVerification::PROCESSOR_DECLINED,
            $result->creditCardVerification->status);
        ...
        $this->assertEquals(Braintree\CreditCard::PREPAID_UNKNOWN,
            $result->creditCardVerification->creditCard['prepaid']);
    }
    ...
    {
        $collection =
            Braintree\CreditCard::expiringBetween(mktime(0, 0, 0, 1, 1, 2010), mktime(23, 59, 59, 12, 31, 2010));
        $this->assertTrue($collection->maximumCount() > 1);
    ...
    public function testUpdate_withCardVerification()
    {
        ...
        $this->assertEquals('I', $result->creditCardVerification->avsPostalCodeResponseCode);
        $this->assertEquals('I', $result->creditCardVerification->avsStreetAddressResponseCode);
        $this->assertEquals(Test\Helper::defaultMerchantAccountId(),
    ...
    public function testUpdate_withCardVerificationAndSpecificMerchantAccount()
    {
        ...
        $this->assertEquals(Braintree\Result\CreditCardVerification::PROCESSOR_DECLINED,
            $result->creditCardVerification->status);
        $this->assertEquals(Test\Helper::nonDefaultMerchantAccountId(),
            $result->creditCardVerification->merchantAccountId);
    }
