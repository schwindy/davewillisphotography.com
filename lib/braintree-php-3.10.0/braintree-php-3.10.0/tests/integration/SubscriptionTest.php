...
class SubscriptionTest extends Setup
{
    ...
    public function testCreate_firstBillingDateInThePast()
    {
        ...
        $this->assertEquals(Braintree\Error\Codes::SUBSCRIPTION_FIRST_BILLING_DATE_CANNOT_BE_IN_THE_PAST,
            $errors[0]->code);
    }
    ...
    public function testCreate_properlyParsesValidationErrorsForArrays()
    {
        ...
        $this->assertFalse($result->success);
        $errors =
            $result->errors->forKey('subscription')->forKey('addOns')->forKey('update')->forIndex(0)->onAttribute('amount');
        $this->assertEquals(Braintree\Error\Codes::SUBSCRIPTION_MODIFICATION_AMOUNT_IS_INVALID, $errors[0]->code);
        $errors =
            $result->errors->forKey('subscription')->forKey('addOns')->forKey('update')->forIndex(1)->onAttribute('quantity');
    ...
    public function testRetryCharge_WithoutAmount()
    {
        ...
        $path =
            Braintree\Configuration::$global->merchantPath() . '/subscriptions/' . $subscription->id . '/make_past_due';
        $http->put($path);
