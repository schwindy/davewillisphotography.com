 ...
 *
 * For more detailed information on CreditCards, see
 * {@link http://www.braintreepayments.com/gateway/credit-card-api http://www.braintreepaymentsolutions.com/gateway/credit-card-api}<br /> For more detailed information on CreditCard verifications, see {@link http://www.braintreepayments.com/gateway/credit-card-verification-api http://www.braintreepaymentsolutions.com/gateway/credit-card-verification-api}
 *
 * @package    Braintree
 * @category   Resources
...
class CreditCardGateway
{
    ...
    public function createFromTransparentRedirect($queryString)
    {
        ...
        return $this->_doCreate('/payment_methods/all/confirm_transparent_redirect_request', ['id' => $params['id']]);
    }
    ...
    {
        $queryPath =
            $this->_config->merchantPath() . '/payment_methods/all/expiring_ids?start=' . date('mY',
    ...
    {
        $queryPath =
            $this->_config->merchantPath() . '/payment_methods/all/expiring?start=' . date('mY',
                $startDate) . '&end=' . date('mY', $endDate);
    ...
    public function credit($token, $transactionAttribs)
    {
        ...
        return Transaction::credit(array_merge($transactionAttribs, ['paymentMethodToken' => $token]));
    }
    ...
    public static function updateSignature()
    {
        ...
        foreach ($signature AS $key => $value) {
            if (is_array($value) and array_key_exists('billingAddress', $value)) {
                $signature[$key]['billingAddress'] =
                    array_merge_recursive($value['billingAddress'], $updateExistingBillingSignature);
            }
        }
